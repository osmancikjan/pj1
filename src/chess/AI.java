...
public class AI {
    ...
    public void MakeMove(ArrayList<ModelObject> objectsOnBoard) {
            ...
            for (Move cat : catches) {
                if (!catches.isEmpty()) {
                    if (cat.GetEvalOf(cat.type) > max) {
                        ...
                        this.FinalFrom = new Point(cat.from);
                        this.FinalTo = new Point(cat.to);
        ...
        catches.removeAll(catches);
        moves.removeAll(moves);
        blacks.clear();
        whites.clear();
        //objectsForMovement.removeAll(objectsForMovement);
    }
